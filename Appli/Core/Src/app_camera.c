...
static void app_camera_display_pipe_init(uint32_t sensor_width, uint32_t sensor_height)
{
    ...
    cmw_dcmipp_conf.output_format = DCMIPP_PIXEL_PACKER_FORMAT_RGB888_YUV444_1;
    cmw_dcmipp_conf.output_bpp = 3;
...
static void app_camera_nn_pipe_init(uint32_t sensor_width, uint32_t sensor_height)
{
    ...
    cmw_dcmipp_conf.output_height = (uint32_t)(NN_HEIGHT * ((float)LCD_BG_HEIGHT / LCD_BG_WIDTH));
